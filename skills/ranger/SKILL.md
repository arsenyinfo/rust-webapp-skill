---
name: ranger
description: Unattended overnight sweep of a named area for tiny bugs, inconsistencies, stale workarounds, and overcomplicated code, shipping the fixes as verified draft PRs. Use when the user asks to run ranger, wants an overnight or unattended sweep, or says to clean an area up while they sleep.
argument-hint: "<path or component to sweep>"
compatibility: "requires an independent review tool (codex:codex-rescue) and the gh CLI; uses the investigate skill for GitHub-issue triage; assumes a git repo with a remote"
---
...
Every simplification finding and every refactor is judged against this posture (it is what "cleaner" means here). Explorers hunt for violations of it; the Codex gate rejects fixes that drift from it. It is language-agnostic — apply the spirit in whatever language the area is written in.
...
- **Abstractions pay rent immediately.** An interface/trait with a single implementation, a `FooManager`/`FooService`/`FooFactory` introduced only to ease testing, premature generics, or indirection added to avoid understanding the data flow — all are findings. This is the /simplify *altitude* lens: collapse the special-case-on-shared-infra tower, don't build it taller.
...
For a genuinely tangled issue, invoke `skill: investigate` on it rather than guessing — the mechanism must be understood before any fix, same as everywhere else in this skill.
...
3. **Overcomplicated / dirty code** — the four /simplify lenses. Two aren't covered by the rubric above and are named here: *reuse* (reimplementing a helper the codebase already has — name it) and *efficiency* (redundant computation, repeated I/O, sequential independent work). The other two, *simplification* and *altitude*, are the *Design taste* rubric applied to structure — redundant state, dead code, stringly/boolean APIs, single-implementation abstractions, layered indirection.
...
5. **Instruction text** (only when the area carries LLM-facing text: system prompts, tool and skill descriptions, agent instructions, schema `description` fields, few-shot examples) — contradictions between instructions, near-duplicate guidance that has drifted apart, references to tools or behavior that no longer exist, and the same concept named differently across surfaces. This text is load-bearing — it steers model behavior — so it is treated like code, not like comments: the explorer flags every candidate, and whether a finding can be *fixed* overnight is decided by the routing rule below. Spawn this lens only when the area actually contains such text.
...
Because these are the riskiest fixes in the sweep, the premise proof is not trusted on your reading alone. At the plan gate (Phase 3), **`codex:codex-rescue` must *independently confirm* the premise is dead** — positively agree the proof holds, not merely fail to refute it — checking the concrete evidence (the newer version exists and resolves the issue, the API is public in the version the project actually uses, the polyfilled feature is available on every supported target, or — for the instruction-text stale references below — the removed referent the text still names, a tool, flag, command, or API, is provably absent). A cluster whose dead-premise proof Codex cannot positively confirm, or is ambiguous on, is downgraded to *needs-your-call*, never shipped on your confidence alone.

Instruction-text findings split by the safety contract exactly as code does. A **stale reference** — a deletion-only mention of a named referent that has been *provably removed*: a tool, flag, command, or API that no longer exists — is stale-reference cleanup, and only when the fix is *removing the dead mention*. It ships overnight under the same hard verification duty and Codex positive-confirmation plan gate as a stale workaround above — that gate is the regression anchor here, standing in for the fail-before/pass-after test. Do not add a string assertion or snapshot proving the removed mention is absent: those pin wording, not behavior. If the repo has a real prompt-eval harness, add an eval showing the model no longer names the referent; absent one, the cleanup ships on the confirmation gate alone. Everything else is a **semantic edit** — merging drifted guidance into one wording, resolving a contradiction, re-ordering which instruction wins, changing what the model is told — and a semantic edit to load-bearing text *is* an observable behavior change: it goes to *needs-your-call*, never an unattended fix, even when one side of it names something old. A contradiction you cannot confidently resolve is reported with both competing instructions quoted, no code.
...
You collect every finding, dedupe, and discard anything unverifiable or speculative — read the cited code yourself before trusting a summary. Dedupe on a stable key (normalized file path + line range + defect mechanism), not on explorer wording, so one defect reported three ways becomes one cluster. Because nothing merges, the run base still contains every defect you already addressed, so also suppress findings whose key matches an already open/shipped draft PR or a key this run already dropped, refuted, or left incomplete — otherwise you re-cluster and re-PR the same defect every round. Suppression is mechanical, not judgment: every draft PR body and every dropped/refuted/needs-your-call/incomplete record in the report carries one `ranger-key: <normalized-path>:<line-range>:<mechanism-slug>` line per finding, so the check is `gh pr list --json body` plus a grep for the key, never fuzzy matching against prose bullets. Promote a finding only when you can reproduce the defect from the cited source without assuming intent. Then:

- **Cluster by theme**, not by file: all instances of one defect kind in one module form one cluster → one branch → one PR. A cluster stays small enough to review in a sitting; split it if it sprawls.
- **Classify each finding — an ordered rubric, first matching row wins:**
  1. *drop* — the defect can't be reproduced from the cited source, or the fix would be cosmetic churn → discard, record its `ranger-key`.
  2. *out-of-scope* — outside `$ARGUMENTS` → report only.
...
The plan gate does not replace the diff gate (Phase 5); it is an earlier, cheaper filter. A cluster must clear **both** — Codex refutes neither the plan nor the diff. **Stale-workaround and instruction-text stale-reference clusters clear the plan gate on the stricter positive-confirmation rule, per the Phase 1 duty.**

## Phase 4 — Implement

Per cluster, on its own branch cut from the pinned run base (Preflight), not from a re-fetched `main`:

- **Trivial-mechanical** clusters go to Sonnet fixer subagents. Each fixer is scoped to only its cluster's files, told to add the regression test, and told not to touch git state, formatting, or dependencies beyond its change. Worktrees are not mandatory — they are the mechanism for running fixers **concurrently** (two branches can't share one working tree), so use one worktree per cluster when you fan fixers out in parallel, and skip them when you run clusters serially (checkout branch → fix → commit → next). Overnight throughput usually favors the parallel path. Track which worktrees this run created (the destructive-command exception depends on it); never reuse a pre-existing worktree, remove a run-created one only once it is clean and its cluster has shipped or dropped, and report any you can't cleanly remove.
- **Subtle** clusters you implement yourself, serially — no worktree needed.
- Every behavioral fix gets its failing-then-passing regression test in the same change. Leave the tree green after each cluster.
...
3. **Codex refutation blocks the PR.** If Codex finds the fix wrong or out-of-scope: rework it and re-verify, or drop the cluster. A refuted fix never ships. If Codex and your review disagree, the more skeptical verdict wins.

**Fail-closed, at this gate and every other:** a verifier timeout, tool failure, or ambiguous verdict counts as a block, not a pass — keep the branch unshipped and send the cluster to *needs-your-call* with the failure noted. Cap rework at two attempts per cluster — a fix that won't pass is dropped, not retried all night. Do not soften this into "advisory": the blocking gate is the reason this is safe to run while you sleep.

## Phase 6 — Ship

For each cluster that clears the gate, open one **draft** PR with `gh`:
- Title names the theme and area. Body lists each finding as `file:line` → what was wrong → what changed, plus the Codex verdict, in a lean bullet list, and one `ranger-key: <normalized-path>:<line-range>:<mechanism-slug>` line per finding for dedup (Phase 2). No process narration, no meta.

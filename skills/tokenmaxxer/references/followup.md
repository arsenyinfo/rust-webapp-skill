...
1. Read the ledger at the router's path convention. Then always cross-check surviving sweep reports (`/tmp/sweep-report-*`) and experiment run files (`/tmp/experiment-*`) — even when the ledger has open entries, since a mirror step can fail for one entry while others landed — and import any *needs-your-call*, ledger-shaped follow-up, or finishable *Incomplete* entry whose `sweep-key` the ledger lacks. If neither source yields `open`/`deferred` entries, report there is nothing to follow up and stop.

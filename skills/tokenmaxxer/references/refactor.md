...
This mode runs the **build** workflow (`build.md`) — its Design, Implementation, and REVIEW loop govern the dev cycle. The sections below specialize those phases for layered-code refactoring; where they conflict, the rules here win.

**Hard default — behavior preservation.** refactor must not change externally observable behavior: API responses, CLI output, persisted formats, error messages callers rely on. This holds through every phase. Any intended behavior change requires explicit user approval before it is implemented (Escalation — `core.md`).
...
2. **Dead-weight inventory**, through the four /simplify lenses (`design-taste.md`). *Altitude* is the primary lens here — accreted layers are usually altitude failures.
3. **Test census**: for each test, what behavior it pins.
   - Mark tests of removed behavior and every kind in the useless-test taxonomy (`deletion-evidence.md`, Useless tests): tautological, trivial, prose-pinning, duplicate, and **over-mocked/wiring tests** — tests whose setup mocks or monkeypatches the component's own internals, or any collaborator that is not a true process boundary (network, clock, subprocesses, externally-owned storage; a tmp dir is real I/O, not a boundary).
...
- **Deletion ledger**: every abstraction, test, comment, flag, or compatibility path to be dropped, each with evidence it is safe per `deletion-evidence.md` (usage/config/test/history; migration-shaped candidates additionally require the history check; "no traffic" requires a traffic source). Categories:

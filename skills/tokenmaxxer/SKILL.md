...
3. Repeat, max 5 rounds. Stop early when a round yields nothing actionable, or when everything still open is P2 or below — never with an open P1. Findings unresolved at the stop are surfaced to the mode's approver, not retried; a P1 still open at the cap keeps the gate closed until the approver rules.

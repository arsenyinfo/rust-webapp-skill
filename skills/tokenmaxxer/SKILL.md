...
compatibility: "requires an external review tool (e.g. codex:codex-rescue or opencode) and the investigate skill; refactor/sweep add design-taste and deletion-evidence references; sweep additionally needs the gh CLI and a git repo with a remote"
---
...
- **`build <task>`** → `references/build.md`. A feature, a root-caused bug fix, or a broad cross-cutting change.
- **`refactor <component>`** → `references/refactor.md`. Consolidate a component that grew by accretion into one coherent design; behavior-preserving.
